...
static FileInfo *file_info_new(const char *file){
	...
	hsize=ms_read_wav_header_from_fd(&header,&rifx,fd);
	if (hsize<=0){
		ms_error("%s: not a wav file", file);
		return NULL;
	}
	if (header.format_chunk.channel<1){
	...
	fi->rate=header.format_chunk.rate;
	fi->nchannels=header.format_chunk.channel;
	fi->nsamples=size/(sizeof(int16_t)*fi->nchannels);
        if (rifx) fi->big_endian = 1;
	fi->fd = fd;
...
int ms_audio_diff(const char *ref_file, const char *matched_file, double *ret, const MSAudioDiffParams *params, MSAudioDiffProgressNotify func, void *user_data){
	...
	*ret=0;
	...
	if (fi1->big_endian!=fi2->big_endian){
		ms_error("Comparing files with different endianess is not supported (%d vs %d)", fi1->big_endian, fi2->big_endian);
		err = -1;
		goto end;
	}
	if (fi1->nsamples == 0){
